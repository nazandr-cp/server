...
// PackCollectionYieldTransferred is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x1e62a02d.
//
// Solidity: function collectionYieldTransferred(address collectionAddress) view returns(uint256)
func (iCollectionsVault *ICollectionsVault) PackCollectionYieldTransferred(collectionAddress common.Address) []byte {
	enc, err := iCollectionsVault.abi.Pack("collectionYieldTransferred", collectionAddress)
	if err != nil {
		panic(err)
	}
	return enc
...
// UnpackCollectionYieldTransferred is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x1e62a02d.
//
// Solidity: function collectionYieldTransferred(address collectionAddress) view returns(uint256)
func (iCollectionsVault *ICollectionsVault) UnpackCollectionYieldTransferred(data []byte) (*big.Int, error) {
	out, err := iCollectionsVault.abi.Unpack("collectionYieldTransferred", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
...
// the contract method with ID 0x8f6fbf09.
//
// Solidity: function repayBorrowBehalfBatch(address[] collectionAddresses, uint256[] amounts, address[] borrowers, uint256 totalAmount) returns()
func (iCollectionsVault *ICollectionsVault) PackRepayBorrowBehalfBatch(collectionAddresses []common.Address, amounts []*big.Int, borrowers []common.Address, totalAmount *big.Int) []byte {
	enc, err := iCollectionsVault.abi.Pack("repayBorrowBehalfBatch", collectionAddresses, amounts, borrowers, totalAmount)
	if err != nil {
		panic(err)
	}
	return enc
...
// PackSetCollectionYieldSharePercentage is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x9bfdf2f7.
//
// Solidity: function setCollectionYieldSharePercentage(address collectionAddress, uint16 percentage) returns()
func (iCollectionsVault *ICollectionsVault) PackSetCollectionYieldSharePercentage(collectionAddress common.Address, percentage uint16) []byte {
	enc, err := iCollectionsVault.abi.Pack("setCollectionYieldSharePercentage", collectionAddress, percentage)
	if err != nil {
		panic(err)
	}
	return enc
